...
define([
  ...
  function(declare, BaseWidget, Directions, Locator, RouteParameters, esriRequest, graphicsUtils,
    ...
    return declare([BaseWidget], {
      ...
      _getGoogle: function(evt){
        array.forEach(evt.target.stops,function(stop){
          if (stop.name.split(",").filter(function(val){return isNaN(val)}).length == 0){
            stop.passVal = "current location"
          }else{
            stop.passVal = stop.name + ", Ottawa, Ontario, Canada"
          }
        })
        window.open("http://maps.apple.com?saddr=" + evt.target.stops[0].passVal + "&daddr=" + evt.target.stops[1].passVal);
      },
      ...
      _preProcessConfig:function(){
        ...
        all([this._getLocatorUrl()]).then(
          lang.hitch(this, function(results){
          this.config.routeTaskUrl = "unused";
          var locatorUrl = results[0];
          this.config.travelModesUrl = "unused";
          esriRequest({
            url: locatorUrl,
            hanleAs:'json',
            content:{
